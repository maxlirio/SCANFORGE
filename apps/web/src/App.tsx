...
import { DEMO_MODEL_URL } from './lib/backend';
import exampleResult from '../public/example/result.json';

type Screen = 'landing' | 'capture' | 'processing' | 'result' | 'example';

/**
 * A real reconstruction produced by this pipeline (30 photographs of a stone
 * statue, COLMAP sparse tier), bundled so the viewer can be tried where there is
 * no backend — a static deployment, for instance. It is labelled as an example
 * everywhere it appears; it is never presented as the result of a live scan.
 */
const EXAMPLE_JOB: JobRecord = {
  id: 'example',
  status: 'succeeded',
  createdAt: 0,
  updatedAt: 0,
  options: { provider: 'colmap-local', quality: 'balanced', mode: 'object' },
  imageCount: exampleResult.photosSubmitted,
  stages: [],
  logs: [],
  files: [{ name: 'model.glb', bytes: exampleResult.glbBytes, primary: true,
            label: 'GLB (geometry + texture in one file)' }],
  result: { ...exampleResult, files: [] } as JobRecord['result'],
};
...
export default function App() {
  ...
  useEffect(() => {
    ...
    else if (id === 'example' && DEMO_MODEL_URL) setScreen('example');
    ...
    <div className="app">
      ...
      {screen === 'landing' && (
        <Landing
          ...
          onViewExample={DEMO_MODEL_URL ? () => setScreen('example') : undefined}
        />
      )}
      ...
      {screen === 'example' && (
        <ResultView
          job={EXAMPLE_JOB}
          fileUrlFor={(name) => `${DEMO_MODEL_URL}${name}`}
          banner={'Example output — generated by TRELLIS.2 from a single photograph of a ' +
            'stone statue, running locally on Apple Silicon. Bundled with the page so the ' +
            'viewer works without a server; it is not a scan you just made. Source ' +
            'photograph: AliceVision dataset_buddha, CC-BY-4.0.'}
